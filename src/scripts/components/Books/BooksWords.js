...
let BooksWords = React.createClass({
  ...
  componentWillMount() {
    ...
    this.props.flux.getActions('books').fetchWords(params).then((res) => {
      console.log('res', res);
...
addWords(){
  ...
  var idPelicula = this.getParams().id;
  this.transitionTo('/addWords/:id/:idPelicula/:idEpisodio', {id: '3', idPelicula: idPelicula, idEpisodio:-1});
},
